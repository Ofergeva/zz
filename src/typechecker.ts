...
export class TypeChecker {
  ...
  private validateFieldType(dataType: DataType, structName: string, line: number): void {
  ...
  private checkStatement(statement: Statement): void {
    switch (statement.type) {
      ...
      case 'BreakStatement':
        if (this.loopDepth === 0) {
          this.errors.push(
            `Break statement (>!) at line ${statement.line} must be inside a loop.`
          );
        }
        break;
      case 'ContinueStatement':
        if (this.loopDepth === 0) {
          this.errors.push(
            `Continue statement (>>) at line ${statement.line} must be inside a loop.`
          );
        }
        break;
      case 'TryStatement':
  ...
  private checkMatchExpression(match: MatchExpression): void {
      ...
      if (allVariants) {
        for (const variant of allVariants) {
          if (!coveredVariants.has(variant)) {
            this.errors.push(
              `Non-exhaustive match at line ${match.line}: missing pattern for ${valueType.name}.${variant}.`
            );
          }
        }
      }
  ...
  private checkPatternType(pattern: Pattern, expected: DataType | null, line: number): void {
    if (!expected) return;
    ...
    switch (pattern.kind) {
      case 'enum':
        if (!isEnumType(expected) || expected.name !== pattern.enumName) {
          this.errors.push(
            `Pattern type mismatch at line ${line}: expected ${this.typeToString(expected)}, got ${pattern.enumName}.`
          );
        }
        ...
        if (variants && !variants.includes(pattern.variant)) {
          this.errors.push(
            `Unknown variant '${pattern.variant}' in enum '${pattern.enumName}' at line ${line}.`
          );
        }
        break;
      case 'struct':
        if (!isStructType(expected) || expected.name !== pattern.structName) {
          this.errors.push(
            `Pattern type mismatch at line ${line}: expected ${this.typeToString(expected)}, got ${pattern.structName}.`
          );
        }
        break;
      case 'tuple':
        if (!isTupleType(expected)) {
          this.errors.push(
            `Cannot use tuple pattern on ${this.typeToString(expected)} at line ${line}.`
          );
        }
        break;
      case 'literal':
        ...
        if (litType && !this.typesCompatible(litType, expected)) {
          this.errors.push(
            `Literal pattern type mismatch at line ${line}: expected ${this.typeToString(expected)}, got ${this.typeToString(litType)}.`
          );
        }
        break;
    }
  ...
  private addPatternBindings(pattern: Pattern, valueType: DataType | null, line: number): void {
    switch (pattern.kind) {
      case 'binding':
        if (valueType) {
  ...
  private checkIndexAssignment(stmt: IndexAssignment): void {
    ...
    if (containerType && isTupleType(containerType)) {
      this.errors.push(
        `Cannot assign to tuple index at line ${stmt.line}. Tuples are immutable.`
      );
      return;
    }
    ...
    if (indexType && isPrimitiveType(indexType) && indexType !== 'int') {
      this.errors.push(
        `Array index must be an integer at line ${stmt.line}, got ${indexType}.`
      );
    }
    ...
    if (containerType && isArrayType(containerType)) {
      ...
      if (valueType && isPrimitiveType(valueType) && valueType !== containerType.elementType) {
        this.errors.push(
          `Type mismatch at line ${stmt.line}: cannot assign ${valueType} to ${containerType.elementType} array element.`
        );
      }
    }
  }

  private checkFieldAssignment(stmt: FieldAssignment): void {
    ...
    if (!objectType || !isStructType(objectType)) {
      this.errors.push(
        `Cannot assign to field '${stmt.field}' on non-struct type at line ${stmt.line}.`
      );
      return;
    }
    ...
    if (stmt.object.type === 'Identifier') {
      ...
      if (varInfo && varInfo.mutability === 'immutable') {
        this.errors.push(
          `Cannot assign to field '${stmt.field}' on immutable struct '${stmt.object.name}' at line ${stmt.line}. ` +
    ...
    if (!structInfo) {
      this.errors.push(
        `Unknown struct '${objectType.name}' at line ${stmt.line}.`
      );
      return;
    }
    ...
    if (!field) {
      this.errors.push(
        `Unknown field '${stmt.field}' on struct '${objectType.name}' at line ${stmt.line}.`
      );
      return;
    }
    ...
    if (valueType && !this.typesCompatible(valueType, field.dataType)) {
      this.errors.push(
        `Type mismatch at line ${stmt.line}: cannot assign ${this.typeToString(valueType)} to ${this.typeToString(field.dataType)} field '${stmt.field}'.`
      );
    }
  }
  ...
  private checkVariableDeclaration(decl: VariableDeclaration): void {
    ...
    if (this.variables.has(decl.name)) {
      ...
      this.errors.push(
        `Variable '${decl.name}' already declared at line ${existing.line}. ` +
    ...
    if (isTupleType(decl.dataType) && decl.mutability === 'mutable') {
      this.errors.push(
        `Tuples must be immutable at line ${decl.line}. Use # instead of ~.`
      );
    }
  ...
  private checkAssignment(assignment: Assignment): void {
    ...
    if (!varInfo) {
      this.errors.push(
        `Undeclared variable '${assignment.name}' at line ${assignment.line}.`
      );
      return;
    }
  ...
  private checkIncrementStatement(stmt: IncrementStatement): void {
    const varInfo = this.variables.get(stmt.name);

    if (!varInfo) {
      this.errors.push(
        `Undeclared variable '${stmt.name}' at line ${stmt.line}.`
      );
      return;
    }
    ...
    if (varInfo.mutability === 'immutable') {
      this.errors.push(
        `Cannot modify immutable variable '${stmt.name}' with ${stmt.operator} at line ${stmt.line}. ` +
        `Variable was declared as immutable (#) at line ${varInfo.line}.`
      );
      return;
    }
    ...
    if (!this.isNumeric(varInfo.dataType)) {
      this.errors.push(
        `Cannot use ${stmt.operator} on ${this.typeToString(varInfo.dataType)} variable '${stmt.name}' at line ${stmt.line}. ` +
        `Operator requires int or float.`
      );
    }
  }

  private checkCompoundAssignment(stmt: CompoundAssignment): void {
    const varInfo = this.variables.get(stmt.name);
    ...
    if (stmt.operator === '+=' && varInfo.dataType === 'string') {
      if (valueType && valueType !== 'string') {
        this.errors.push(
          `Cannot concatenate ${this.typeToString(valueType)} to string variable '${stmt.name}' at line ${stmt.line}. Use s() to convert.`
        );
      }
      return;
    }
    ...
    if (valueType && !this.isNumeric(valueType)) {
      this.errors.push(
        `Type mismatch at line ${stmt.line}: cannot use ${stmt.operator} with ${this.typeToString(valueType)} value.`
      );
    }
  }

  private checkPrintStatement(print: PrintStatement): void {
    // Just verify the expression is valid
    this.checkExpression(print.expression, print.line);
  }

  private checkErrorStatement(error: ErrorStatement): void {
    // Just verify the expression is valid
    this.checkExpression(error.expression, error.line);
  }

  private checkThrowStatement(stmt: ThrowStatement): void {
    // Just verify the expression is valid
  ...
  private checkForStatement(stmt: ForStatement): void {
    ...
    if (startType && isPrimitiveType(startType) && startType !== 'int') {
      this.errors.push(
        `For loop range start must be an integer at line ${stmt.line}, got ${startType}.`
      );
    }
    if (endType && isPrimitiveType(endType) && endType !== 'int') {
      this.errors.push(
        `For loop range end must be an integer at line ${stmt.line}, got ${endType}.`
      );
    }
  ...
  private checkForEachStatement(stmt: ForEachStatement): void {
    this.checkExpression(stmt.iterable, stmt.line);

    const iterableType = this.inferExpressionType(stmt.iterable);

    let elementType: DataType = 'string'; // fallback
    if (iterableType) {
      if (isArrayType(iterableType)) {
        elementType = iterableType.elementType;
      } else {
        this.errors.push(
          `For-each loop requires an array at line ${stmt.line}, got ${typeof iterableType === 'string' ? iterableType : iterableType.kind}.`
        );
      }
    }
  ...
  private checkImportStatement(stmt: ImportStatement): void {
  ...
  private checkFunctionDeclaration(decl: FunctionDeclaration): void {
    ...
    if (this.functions.has(decl.name)) {
      ...
      this.errors.push(
        `Function '${decl.name}' already declared at line ${existing.line}. ` +
        `Cannot redeclare at line ${decl.line}.`
      );
      return;
    }
    ...
    if (decl.returnExpression) {
      ...
      if (returnExprType && decl.returnType !== 'void' && !this.typesCompatible(returnExprType, decl.returnType)) {
        this.errors.push(
          `Type mismatch at line ${decl.line}: function '${decl.name}' should return ${this.typeToString(decl.returnType)}, but returns ${this.typeToString(returnExprType)}.`
        );
      }
      ...
      if (!hasJsBlock) {
        this.errors.push(
          `Function '${decl.name}' at line ${decl.line} has return type ${decl.returnType} but no return expression.`
        );
      }
    }
  ...
  private checkFunctionCall(call: FunctionCall, line: number): void {
    ...
    if (!funcInfo) {
      this.errors.push(
        `Undeclared function '${call.name}' at line ${line}.`
      );
      return;
    }
    ...
    const providedParams = new Map<string, Expression>();

    // Process positional arguments first
    for (let i = 0; i < positionalArgs.length; i++) {
      if (i >= funcInfo.parameters.length) {
        this.errors.push(
          `Too many arguments for function '${call.name}' at line ${line}. Expected ${funcInfo.parameters.length}.`
        );
        break;
      }
      ...
      if (argType && !this.typesCompatible(argType, param.dataType)) {
        this.errors.push(
          `Type mismatch at line ${line}: argument ${i + 1} to '${call.name}' should be ${this.typeToString(param.dataType)}, got ${this.typeToString(argType)}.`
        );
      }
    }
    ...
    for (const arg of namedArgs) {
      ...
      if (!param) {
        this.errors.push(
          `Unknown parameter '${arg.name}' for function '${call.name}' at line ${line}.`
        );
        continue;
      }
      if (providedParams.has(arg.name!)) {
        this.errors.push(
          `Parameter '${arg.name}' already provided for function '${call.name}' at line ${line}.`
        );
        continue;
      }
      ...
      if (argType && !this.typesCompatible(argType, param.dataType)) {
        this.errors.push(
          `Type mismatch at line ${line}: argument '${arg.name}' to '${call.name}' should be ${this.typeToString(param.dataType)}, got ${this.typeToString(argType)}.`
        );
      }
    }
    ...
    for (const param of funcInfo.parameters) {
      if (!providedParams.has(param.name)) {
        this.errors.push(
          `Missing argument '${param.name}' for function '${call.name}' at line ${line}.`
        );
      }
    }
  }

  private checkExpression(expr: Expression, line: number): void {
    if (expr.type === 'Identifier') {
      if (!this.variables.has(expr.name)) {
        this.errors.push(
          `Undeclared variable '${expr.name}' at line ${line}.`
        );
      }
      ...
      if (expr.operator === '&&' || expr.operator === '||') {
        if (leftType !== null && leftType !== 'bool') {
          this.errors.push(
            `Operator '${expr.operator}' requires boolean operands at line ${line}, got ${this.typeToString(leftType)} on left side.`
          );
        }
        if (rightType !== null && rightType !== 'bool') {
          this.errors.push(
            `Operator '${expr.operator}' requires boolean operands at line ${line}, got ${this.typeToString(rightType)} on right side.`
          );
        }
        return;
      }
      ...
      if (this.isComparisonOperator(expr.operator)) {
        ...
        if (leftType && rightType) {
          ...
          if (!bothNumeric && !sameType) {
            this.errors.push(
              `Cannot compare ${this.typeToString(leftType)} with ${this.typeToString(rightType)} at line ${line}.`
            );
          }
        }
        return;
      ...
      if (expr.operator === '+') {
        ...
        if (leftIsString || rightIsString) {
          ...
          if (!leftIsString && leftType) {
            this.errors.push(
              `Cannot concatenate string with ${leftType} at line ${line}. Use s() to convert.`
            );
          }
          if (!rightIsString && rightType) {
            this.errors.push(
              `Cannot concatenate string with ${rightType} at line ${line}. Use s() to convert.`
            );
          }
          return;
        }
      ...
      if (leftType && !this.isNumeric(leftType)) {
        this.errors.push(
          `Cannot use operator '${expr.operator}' on ${leftType} at line ${line}.`
        );
      }
      if (rightType && !this.isNumeric(rightType)) {
        this.errors.push(
          `Cannot use operator '${expr.operator}' on ${rightType} at line ${line}.`
        );
      }
      ...
      if (expr.operator === '!') {
        if (operandType !== null && operandType !== 'bool') {
          this.errors.push(
            `Operator '!' requires boolean operand at line ${line}, got ${this.typeToString(operandType)}. Use an explicit comparison (e.g., x == 0, x == _).`
          );
        }
        return;
      }
      ...
      if (operandType && !this.isNumeric(operandType)) {
        this.errors.push(
          `Cannot use unary '${expr.operator}' on ${operandType} at line ${line}.`
        );
      }
        ...
        if (firstType && isPrimitiveType(firstType)) {
          for (let i = 1; i < expr.elements.length; i++) {
            ...
            if (elemType && elemType !== firstType) {
              this.errors.push(
                `Tuple elements must all have the same type at line ${line}. ` +
      ...
      if (startType && isPrimitiveType(startType) && startType !== 'int') {
        this.errors.push(
          `Range start must be an integer at line ${line}, got ${startType}.`
        );
      }
      if (endType && isPrimitiveType(endType) && endType !== 'int') {
        this.errors.push(
          `Range end must be an integer at line ${line}, got ${endType}.`
        );
      }
      ...
      if (indexType && isPrimitiveType(indexType) && indexType !== 'int') {
        this.errors.push(
          `Array index must be an integer at line ${line}, got ${indexType}.`
        );
      }
      ...
      if (objectType === 'string') {
        ...
        if (stringMethods.includes(expr.method)) {
          ...
          if (expr.method === 'len' && expr.arguments.length > 0) {
            this.errors.push(
              `String method 'len' takes no arguments at line ${line}.`
            );
          }
          if (expr.method === 'at') {
            if (expr.arguments.length !== 1) {
              this.errors.push(
                `String method 'at' requires exactly 1 argument at line ${line}.`
              );
            } else {
              const argType = this.inferExpressionType(expr.arguments[0]);
              if (argType && argType !== 'int') {
                this.errors.push(
                  `String method 'at' requires int argument at line ${line}, got ${this.typeToString(argType)}.`
                );
              }
            }
          }
          ...
          if (funcInfo) {
            ...
            this.errors.push(
              `Unknown string method '${expr.method}' at line ${line}. Built-in methods: ${stringMethods.join(', ')}. Or import a function with this name.`
            );
          }
        }
      }
      ...
      // Validate method exists for tuples
        ...
        if (!validMethods.includes(expr.method)) {
          ...
          if (!funcInfo) {
            this.errors.push(
              `Invalid method '${expr.method}' on tuple at line ${line}. Tuples only support: ${validMethods.join(', ')}.`
            );
          }
        } else {
          ...
          if (expr.method === 'len' && expr.arguments.length > 0) {
            this.errors.push(
              `Tuple method '${expr.method}' takes no arguments at line ${line}.`
            );
          }
        }
      }
      ...
      // Validate method exists for arrays
        ...
        if (!validMethods.includes(expr.method)) {
          ...
          if (!funcInfo) {
            this.errors.push(
              `Unknown array method '${expr.method}' at line ${line}. Valid methods: ${validMethods.join(', ')}.`
            );
          }
        } else {
          ...
          if (objectType.size !== undefined && (expr.method === 'push' || expr.method === 'pop')) {
            this.errors.push(
              `Cannot use '${expr.method}' on fixed-size array at line ${line}. Array was declared with size ${objectType.size}.`
            );
          }
          ...
          if (expr.method === 'push') {
            if (expr.arguments.length !== 1) {
              this.errors.push(
                `Array method 'push' requires exactly 1 argument at line ${line}.`
              );
            } else {
              const argType = this.inferExpressionType(expr.arguments[0]);
              if (argType && isPrimitiveType(argType) && argType !== objectType.elementType) {
                this.errors.push(
                  `Type mismatch at line ${line}: cannot push ${argType} to ${objectType.elementType} array.`
                );
              }
            }
          }
          ...
          if ((expr.method === 'len' || expr.method === 'pop') && expr.arguments.length > 0) {
            this.errors.push(
              `Array method '${expr.method}' takes no arguments at line ${line}.`
            );
          }
        }
      }
        ...
        if (structInfo) {
          ...
          if (methodInfo) {
            ...
            if (expr.arguments.length !== methodInfo.parameters.length) {
              this.errors.push(
                `Method '${expr.method}' on struct '${objectType.name}' expects ${methodInfo.parameters.length} arguments, got ${expr.arguments.length} at line ${line}.`
              );
            } else {
              for (let i = 0; i < expr.arguments.length; i++) {
                ...
                if (argType && !this.typesCompatible(argType, paramType)) {
                  this.errors.push(
                    `Type mismatch at line ${line}: argument ${i + 1} to method '${expr.method}' should be ${this.typeToString(paramType)}, got ${this.typeToString(argType)}.`
                  );
                }
              }
            }
            ...
            if (!funcInfo) {
              this.errors.push(
                `Unknown method '${expr.method}' on struct '${objectType.name}' at line ${line}.`
              );
            }
          }
        }
      ...
      // UFCS for other types: check if there's a function with this name
        ...
        if (!funcInfo) {
          this.errors.push(
            `Unknown method '${expr.method}' on ${this.typeToString(objectType)} at line ${line}.`
          );
        }
      }
      ...
      if (objectType && isStructType(objectType)) {
        ...
        if (structInfo) {
          ...
          if (!field) {
            this.errors.push(
              `Unknown field '${expr.property}' on struct '${objectType.name}' at line ${line}.`
            );
          }
        }
      }
      ...
      if (!variants) {
        this.errors.push(
          `Unknown enum '${expr.enumName}' at line ${line}.`
        );
  ...
  private checkStructInstantiation(expr: StructInstantiation, line: number): void {
    const structInfo = this.structs.get(expr.structName);
    if (!structInfo) {
      this.errors.push(
        `Unknown struct '${expr.structName}' at line ${line}.`
      );
      return;
    }
    ...
    const providedFields = new Map<string, Expression>();

    // Process positional arguments first
    for (let i = 0; i < positionalArgs.length; i++) {
      if (i >= structInfo.fields.length) {
        this.errors.push(
          `Too many arguments for struct '${expr.structName}' at line ${line}. Expected ${structInfo.fields.length}.`
        );
        break;
      }
      ...
      if (argType && !this.typesCompatible(argType, field.dataType)) {
        this.errors.push(
          `Type mismatch at line ${line}: field '${field.name}' of struct '${expr.structName}' expects ${this.typeToString(field.dataType)}, got ${this.typeToString(argType)}.`
        );
      }
    }
    ...
    for (const arg of namedArgs) {
      ...
      if (!field) {
        this.errors.push(
          `Unknown field '${arg.name}' for struct '${expr.structName}' at line ${line}.`
        );
        continue;
      }
      if (providedFields.has(arg.name!)) {
        this.errors.push(
          `Field '${arg.name}' already provided for struct '${expr.structName}' at line ${line}.`
        );
        continue;
      }
      ...
      if (argType && !this.typesCompatible(argType, field.dataType)) {
        this.errors.push(
          `Type mismatch at line ${line}: field '${arg.name}' of struct '${expr.structName}' expects ${this.typeToString(field.dataType)}, got ${this.typeToString(argType)}.`
        );
      }
    }
    ...
    for (const field of structInfo.fields) {
      if (!providedFields.has(field.name)) {
        this.errors.push(
          `Missing field '${field.name}' for struct '${expr.structName}' at line ${line}.`
        );
      }
    }
  }
  ...
  private requireBooleanCondition(expr: Expression, line: number, context: string): void {
    ...
    if (exprType !== null && exprType !== 'bool') {
      this.errors.push(
        `${context} condition must be boolean at line ${line}, got ${this.typeToString(exprType)}. Use an explicit comparison (e.g., x > 0, x != _).`
      );
    }
  }
  ...
  private typesEqual(a: DataType | null, b: DataType | null | 'void'): boolean {
  ...
  private typeToString(type: DataType | null | 'void'): string {
  ...
  private inferExpressionType(expr: Expression): DataType | null {
    switch (expr.type) {
      case 'StringLiteral':
        return 'string';
