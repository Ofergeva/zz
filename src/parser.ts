...
export class Parser {
  ...
  private parseWhileStatement(): WhileStatement | ForStatement {
  ...
  private parseForLoop(loopToken: Token): ForStatement {
  ...
  private parseEnumVariableDeclaration(exported: boolean = false): VariableDeclaration {
    ...
    const dataType: EnumType = { kind: 'enum', name: enumName };
  ...
  private parseStructVariableDeclaration(exported: boolean = false): VariableDeclaration {
    ...
    const dataType: StructType = { kind: 'struct', name: structName };
  ...
  private parseCastExpression(): CastExpression {
    ...
    if (this.isTupleCastToken(token.type)) {
      ...
      switch (token.type) {
        case TokenType.CAST_STRING: targetType = 'string'; break;
        case TokenType.CAST_INT: targetType = 'int'; break;
        case TokenType.CAST_FLOAT: targetType = 'float'; break;
        case TokenType.CAST_BOOL: targetType = 'bool'; break;
