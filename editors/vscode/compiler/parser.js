...
export class Parser {
    ...
    traitNames = new Set(); // Track known trait names for type resolution
    typeParamNames = new Set(); // Track active type parameters for generic structs/functions
    parseDepth = 0; // Track recursion depth to prevent stack overflow
    ...
    enterParse(context) {
        ...
        if (this.parseDepth > MAX_PARSER_DEPTH) {
            ...
            throw new Error(`Maximum parse depth (${MAX_PARSER_DEPTH}) exceeded at line ${token?.line ?? 1}, column ${token?.column ?? 1}. ` +
    ...
    parseTraitDeclaration(exported = false) {
    ...
    parseReturnType() {
        ...
        if (this.isTypeToken(token.type)) {
            ...
            let dataType = typeMap[token.type] || "int";
            // Check for array return type
            if (this.peek().type === TokenType.LBRACKET) {
                this.advance(); // consume [
                let size;
        ...
        if (token.type === TokenType.IDENTIFIER && this.structNames.has(token.value)) {
            this.advance();
            let dataType = { kind: "struct", name: token.value };
            if (this.peek().type === TokenType.LBRACKET) {
                this.advance();
                let size;
                if (this.peek().type === TokenType.NUMBER_LITERAL) {
        ...
        if (token.type === TokenType.IDENTIFIER && this.enumNames.has(token.value)) {
            this.advance();
            let dataType = { kind: "enum", name: token.value };
            if (this.peek().type === TokenType.LBRACKET) {
                this.advance();
                let size;
                if (this.peek().type === TokenType.NUMBER_LITERAL) {
        ...
        if (token.type === TokenType.IDENTIFIER && this.traitNames.has(token.value)) {
            this.advance();
            let dataType = { kind: "trait", name: token.value };
            if (this.peek().type === TokenType.LBRACKET) {
                this.advance();
                let size;
                if (this.peek().type === TokenType.NUMBER_LITERAL) {
    ...
    parseTraitParameter() {
        ...
        if (this.isTypeToken(paramTypeToken.type)) {
            ...
            this.advance();
            const structType = { kind: "struct", name: paramTypeToken.value };
            if (this.peek().type === TokenType.LBRACKET) {
                this.advance();
                let size;
                if (this.peek().type === TokenType.NUMBER_LITERAL) {
            ...
            this.advance();
            const enumType = { kind: "enum", name: paramTypeToken.value };
            if (this.peek().type === TokenType.LBRACKET) {
                this.advance();
                let size;
                if (this.peek().type === TokenType.NUMBER_LITERAL) {
            ...
            this.advance();
            const traitType = { kind: "trait", name: paramTypeToken.value };
            if (this.peek().type === TokenType.LBRACKET) {
                this.advance();
                let size;
                if (this.peek().type === TokenType.NUMBER_LITERAL) {
    ...
    parseTraitVariableDeclaration(exported = false) {
    ...
    isStructOrEnumType(token) {
        return (token.type === TokenType.IDENTIFIER && (this.structNames.has(token.value) || this.enumNames.has(token.value) || this.traitNames.has(token.value)));
    }
    parseStructField() {
        ...
        // Check for struct or enum type
            ...
            if (this.structNames.has(typeToken.value)) {
                let structType = { kind: "struct", name: typeToken.value };
    ...
    parseStructMethod() {
        ...
        while (this.peek().type !== TokenType.RPAREN) {
            ...
            // Check for tuple type: ti3#t or ti3[]#tuples
                ...
                const tupleType = { kind: "tuple", elementType: tupleElementType, length: tupleLength };
    ...
    parseStructVariableDeclaration(exported = false) {
        ...
        let structType = { kind: "struct", name: structName };
        if (this.peek().type === TokenType.LT) {
            this.advance(); // consume <
            structType.typeArguments = this.parseTypeArgumentList();
        }
    ...
    parseFunctionDeclaration(exported = false) {
        ...
        if (this.peek().type === TokenType.IDENTIFIER && this.typeParamNames.has(this.peek().value)) {
            ...
            const typeParamType = { kind: "typeParameter", name: typeToken.value };
        ...
        // Check for enum return type: Color Z or Color[] Z
            ...
            const typeToken = this.advance();
            startToken = typeToken;
            const enumType = { kind: "enum", name: typeToken.value };
        ...
        // Check for struct return type: Person Z or Person[] Z
            ...
            const typeToken = this.advance();
            startToken = typeToken;
            const structType = { kind: "struct", name: typeToken.value };
        ...
        // Check for trait return type: Printable Z or Printable[] Z
            ...
            const typeToken = this.advance();
            startToken = typeToken;
            const traitType = { kind: "trait", name: typeToken.value };
            if (this.peek().type === TokenType.LBRACKET) {
                this.advance();
                let size;
                if (this.peek().type === TokenType.NUMBER_LITERAL) {
        ...
        while (this.peek().type !== TokenType.RPAREN) {
            ...
            if (paramTypeToken.type === TokenType.IDENTIFIER && this.typeParamNames.has(paramTypeToken.value)) {
                this.advance();
                const typeParamType = { kind: "typeParameter", name: paramTypeToken.value };
        ...
        while (this.peek().type !== TokenType.SEMICOLON && !this.isAtEnd()) {
            ...
            const isStructArrayDecl = token.type === TokenType.IDENTIFIER &&
                this.structNames.has(token.value) &&
                nextType === TokenType.LBRACKET &&
                this.isArrayVariableDeclaration();
            const isEnumArrayDecl = token.type === TokenType.IDENTIFIER &&
                this.enumNames.has(token.value) &&
                nextType === TokenType.LBRACKET &&
                this.isArrayVariableDeclaration();
            const isTraitArrayDecl = token.type === TokenType.IDENTIFIER &&
    ...
    parseTypeExpression() {
        ...
        if (token.type === TokenType.IDENTIFIER && this.typeParamNames.has(token.value)) {
            ...
            let dataType = { kind: "typeParameter", name: paramName };
            // Check for array: T[]
            if (this.peek().type === TokenType.LBRACKET) {
                this.advance();
                let size;
        ...
        if (token.type === TokenType.IDENTIFIER) {
            ...
            if (this.structNames.has(token.value)) {
                let structType = { kind: "struct", name: token.value };
                ...
                const enumType = { kind: "enum", name: token.value };
                // Check for array
                if (this.peek().type === TokenType.LBRACKET) {
                    this.advance();
                    let size;
                ...
                const traitType = { kind: "trait", name: token.value };
                if (this.peek().type === TokenType.LBRACKET) {
                    this.advance();
                    let size;
                    if (this.peek().type === TokenType.NUMBER_LITERAL) {
    ...
    isGenericTypeArgsFollowedBy(expectedTokens) {
        const savedPos = this.pos;
        try {
    ...
    parseCompTimeFunctionDeclaration() {
